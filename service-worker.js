...
function getStrategyWithFallback(config) {
  ...
  if (strategies.length === 1) {
    ...
    const fallbackStrategy = getFallbackStrategy(config.configuration?.fallback || 'none');
    ...
    return {
      ...
      async querySameTabs(tab) {
        ...
        return allTabs.filter(t => {
          ...
          return tCustomTitle === customTitle || tFallbackTitle === fallbackTitle;
        });
      },
 ...
 */
async function addTabsToGroup(tabs, groupTitle, groupColor, existingGroupId = -1) {
  ...
  if (targetGroupId === -1) {
    ...
    try {
      ...
      console.log(`[TabGroupManager] Created new group ${targetGroupId} for "${groupTitle}"`);
  ...
  if (tabs.length > 1) {
    ...
    try {
      ...
      console.log(`[TabGroupManager] Added ${tabs.length - 1} more tabs to group ${targetGroupId}`);
    } catch (error) {
...
chrome.tabs.onCreated.addListener(async (tab) => {
  ...
  try {
    ...
    console.log(`[TabGroupManager] Added tab ${tab.id} to group ${openerTab.groupId} (inherited from opener ${tab.openerTabId})`);
  } catch (error) {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  console.log('[TabGroupManager] Group title:', groupTitle, 'color:', groupColor);
    ...
    if (existingGroup) {
      ...
      if (otherTabIds.length > 0) {
        try {
          ...
          console.error('[TabGroupManager] Failed to add tabs to existing group:', error);
        }
      }
    } else {
      // Create new group
  ...
  const handleAsync = async () => {
    switch (message.type) {
      ...
      case 'GROUP_TAB': {
        ...
        if (totalTabs.length < config.groupTabNum) {
          return { success: false, error: `Need ${config.groupTabNum} tabs to form a group, found ${totalTabs.length}` };
        }

        const groupTitle = strategy.getGroupTitle(tab);
        const groupColor = strategy.getGroupColor(tab);

        const groupId = await addTabsToGroup(totalTabs, groupTitle, groupColor, -1);
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  switch (command) {
    ...
    case 'group-current-tab': {
      ...
      if (totalTabs.length < config.groupTabNum) {
        console.log(`[TabGroupManager] Need ${config.groupTabNum} tabs, found ${totalTabs.length}`);
        break;
      }
