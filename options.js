...
function setupEventListeners() {
  document.getElementById('enable-auto-group').addEventListener('change', (e) => {
...
function updateUI(config) {
  ...
  document.getElementById('inherit-parent-group').checked = config.inheritParentGroup;
  ...
  if (config.configuration) {
    document.getElementById('fallback-strategy').value = config.configuration.fallback || 'none';
    ...
    <div class="rule-card-header">
      ...
      <select class="rule-color">
        ${['grey', 'blue', 'red', 'yellow', 'green', 'pink', 'purple', 'cyan', 'orange']
          .map(c => `<option value="${c}" ${rule.color === c ? 'selected' : ''}>${c}</option>`)
          .join('')}
      </select>
      <button type="button" class="remove-rule-btn">Remove</button>
    </div>
    <div class="patterns-list">
      ${(rule.patterns || []).map((p, pIndex) => `
